...
def scrape(query):
    ...
    for tweet in soup.select('li[data-item-type="tweet"]'):
        ...
        try:
            url_user = 'https://www.twitter.com/' + ''.join(list(meta_data['data-permalink-path'])).split('/')[1]
            req_user = requests.get(url_user, headers={'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.128 Safari/537.36'})
            ...
            location = tt[0].a.string.split(',')[-1]
        except:
            pass # let go.
